package br.senai.sc.model.persistencia;
import br.senai.sc.model.util.JPAUtil;
import br.senai.sc.model.persistencia.dao.DAO;
import java.util.List;
import javax.persistence.EntityManager;
import javax.persistence.NoResultException;
import javax.swing.JOptionPane;

 /**
 * Interface DAOJPA principal responsável por fazer ligação com
 * transação de dados para o banco, com métodos de Salvar,
 * Editar, Remover, Listar Todos, Listar por Id e uma ligação com 
 * a conexão util.
 * 
 * @author Bruna Zakrzeski
 * @since 1.0
 * @version 1.0 07/08/2014
 */

public abstract class DAOJPA<T, I> implements DAO<T, I> {
    
    //Instanciando com uma variável a conexão do JPAUtil
    private JPAUtil conexao;
    
    
    
    //Método para salvar
    @Override
    public T save(T entity) {
        T saved = entity;

        //Try catch para dizer ao usuário qual o erro
        try {

            //Abrir a transação
            getEntityManager().getTransaction().begin();

            //Persistir a entidade na base de dados
            saved = getEntityManager().merge(saved);

            //Confirma a transação
            getEntityManager().getTransaction().commit();


        } catch (Exception e) {
            //Verificar se a transação está ativa
            if (getEntityManager().getTransaction().isActive() == false) {
                getEntityManager().getTransaction().begin();
            }

            //Desfazer a transação roolback
            getEntityManager().getTransaction().rollback();
            JOptionPane.showMessageDialog(null, "Erro ao salvar um elemento na base"
                    + "de dados! " + e.getMessage());

        }
        return saved;
    }
    
    
    
    
    
  
  
    //Método para remover
    @Override
    public boolean remove(Class<T> classe, I pk) {
        boolean estado = false;

        try {

            getEntityManager().getTransaction().begin();
            getEntityManager().remove(getEntityManager().getReference(classe, pk));
            getEntityManager().getTransaction().commit();
            estado = true;

        } catch (Exception e) {
            if (getEntityManager().getTransaction().isActive() == false) {
                getEntityManager().getTransaction().begin();
            }

            getEntityManager().getTransaction().rollback();
            JOptionPane.showMessageDialog(null, "Erro ao remover elemento"
                    + "na base de dados! " + e.getMessage());
        }

        return estado;
    }

    
    
    
    
    
     
    //Método para listar todos
    @Override
    public List<T> getAll(Class<T> classe) {
        return getEntityManager().createQuery("select x from "
                + classe.getSimpleName() + " x ").getResultList();
    }

    
    
    
    
    
    
    //Método para listar por ID
    @Override
    public T getById(Class<T> classe, I pk) {

        try {

            return getEntityManager().find(classe, pk);
        } catch (NoResultException e) {

            return null;
        }
    }

    
    //Método para criar a conexão com JPAUtil
    @Override
        public EntityManager getEntityManager(){
         if (conexao == null){
             conexao = new JPAUtil();
         }
    
         return conexao.getEntityManager();

    }
    
}
